...
from controllers.sliding_mode_controller import SlidingModeController
from controllers.pole_placement_controller import PolePlacementController
from controllers.mpc_controller import MPController
...
class ControllerManager:
	def __init__(self, robot, controller_params: dict, thruster_system: ThrusterSystem):
		...
		self.mission = robot.mission

		if self.mission.type != "trajectory":
			self.desired_etas = self.mission.waypoints
		...
		elif controller_params['type'] == 'MPC':
			self.controller = MPController(robot, controller_params)
		elif controller_params['type'] == 'PPC':
			self.controller = PolePlacementController(robot, controller_params)
	...
	def manage_waypoint(self, eta, nu):
		...
		if self.mission.type == "trajectory":
			self.desired_eta = self.mission.traj_pos(self.robot.time)
			self.desired_nu = self.mission.traj_vel(self.robot.time)
		else:
